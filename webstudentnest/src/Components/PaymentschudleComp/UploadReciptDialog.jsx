...
import { UseUploadRecipt } from "../../Hooks/StudentHooks/UseUploadRecipt";

export default function UploadReciptFlagDialog({uploadReciptFlag,handleChangeuploadReciptFlag,paymentId,leaseId}) {
  ...
  const {UploadRecipt,loader,error}=UseUploadRecipt();
  const [ReciptImage, setReciptImage] = useState(null);

  function handleDocumentImageChange(e) {
    setReciptImage(e.target.files[0]);
  }

  function handleSubmit(e) {
    e.preventDefault();
    if (ReciptImage) {
    UploadRecipt(paymentId,leaseId,ReciptImage,handleChangeuploadReciptFlag);
    }
  }

  return (
    <Dialog
      open={uploadReciptFlag}
      onClose={handleChangeuploadReciptFlag}
      fullWidth
      maxWidth="sm"
      PaperProps={{
        sx: {
    ...
    >
      ...
      <DialogContent sx={{ mt: 2 }}>
        <form id="add-document-form" className="space-y-4" onSubmit={handleSubmit}>
          ...
          >
            {ReciptImage ? "Change Image" : "Upload Image"}
            <input
              hidden
              accept="image/*"
              type="file"
          ...
          {ReciptImage && (
            <div className="mt-3 flex justify-center">
              <img
                src={URL.createObjectURL(ReciptImage)}
                alt="REcipt-preview"
      ...
      <DialogActions sx={{ justifyContent: "space-between", p: 2 }}>
        <Button
          onClick={handleChangeuploadReciptFlag}
          disabled={loader}
