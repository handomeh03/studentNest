    ...
    <tr key={lease?.leaseId} className="hover:bg-gray-50/80 transition-colors border-b border-gray-100 last:border-none">
      ...
      <td className="whitespace-nowrap px-4 py-5 text-sm">
        <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${
          ...
          : "bg-red-100 text-orange-700 border border-orange-200"
        }`}>
          {lease?.LeaseActivationStatus ? "Active" : "Pending"}
        </span>
      </td>
      ...
      <td className="whitespace-nowrap px-4 py-5 text-sm text-gray-600">{lease?.houseRule} </td>
      ...
      {user?.user?.role=="landlord" || user?.user?.role=="student"?  <td className="whitespace-nowrap px-4 py-5 text-sm">
          <button
            ...
            className="cursor-pointer bg-indigo-600 text-white px-4 py-2 rounded-lg text-xs font-bold shadow-sm hover:bg-indigo-700 hover:shadow-indigo-200 transition-all active:scale-95"
          >
            Payment Schedule
          </button>
        </td>:""}
